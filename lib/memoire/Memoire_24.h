/* Authors: Isidore Lauzier et Jérôme Collin
* Date : 2022-09

* Name of file : Memoire_24.h
* Description : Memory management.
* Material components used : external eeprom.
*/
...
public:
    ...
    static Memoire24CXXX& getInstance();
    ...
    template<typename T>
    T readNext(uint16_t& readAddress) {
        uint8_t byteBuffer[sizeof(T)];
        T result;
        // read in little endian by placing data in reverse order
        for(uint16_t i = 0; i < sizeof(T); ++i)
            lecture(readAddress++, &byteBuffer[sizeof(T) - 1 - i]);
        memcpy(&result, byteBuffer, sizeof(T));
        return result;
    }
private:
    static Memoire24CXXX instance_;
